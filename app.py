...
st.markdown("""
    ...
    /* ESTILO PARA CENTRALIZAÇÃO TOTAL DOS STATUS */
    .status-container {
        display: flex;
    ...
    if menu == "🏠 Home":
        ...
        if st.session_state["modo_operacao"] == "MANUAL":
            ...
            with c1:
                ...
                st.markdown(f"<div class='status-container'>{'<span class=\"blink\">🟢</span>' if status_real == 'ON' else '⚪'}</div>", unsafe_allow_html=True)
            ...
            with c2:
                ...
                st.markdown("<div class='status-container'>💤</div>", unsafe_allow_html=True)
            ...
            with c3:
                ...
                st.markdown(f"<div class='status-container'>{'<span class=\"blink\">🔴</span>' if status_real == 'OFF' else '⚪'}</div>", unsafe_allow_html=True)
...
# ASB AUTOMAÇÃO INDUSTRIAL - v63.0
