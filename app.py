...
# --- 1. CONFIGURAÇÃO VISUAL INTEGRAL (v21.0 - BASE v13.0 + CICLO AUTO + SIMETRIA) ---
...
st.markdown("""
    ...
    /* BOTÕES COM TAMANHO PADRONIZADO E SIMÉTRICO - AJUSTADO v21.0 */
    div.stButton > button {
        width: 100% !important;
...
# --- 2. FUNÇÕES DE NÚCLEO (v13.0 PRESERVADAS) ---
...
if not st.session_state["logado"]:
    ...
    if menu == "🏠 Home":
        ...
        else: # AUTOMÁTICO v21.0
            st.info("🤖 MODO AUTOMÁTICO: O ciclo deve ser iniciado pelo operador.")
            col_t1, col_t2 = st.columns(2)
            with col_t1: t_trabalho = st.number_input("Tempo de Trabalho (minutos)", min_value=1, value=5)
            with col_t2: t_pisca = st.number_input("Velocidade do Pisca (segundos)", min_value=1, value=2)
            
            if not st.session_state["ciclo_ativo"]:
                if st.button("▶️ INICIAR CICLO OPERACIONAL"):
                    st.session_state["ciclo_ativo"] = True; registrar_evento("INICIOU CICLO AUTO"); st.rerun()
            else:
                if st.button("⏹️ PARAR CICLO"):
                    st.session_state["ciclo_ativo"] = False; db.reference("controle/led").set("OFF"); st.rerun()
...
# ASB AUTOMAÇÃO INDUSTRIAL - v21.0
