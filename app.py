...
st.markdown("""
    ...
    /* ESTILO BASE DOS BOTÕES - UNIFORMIDADE TOTAL */
    div.stButton > button {
        width: 100% !important;
    ...
    /* ANIMAÇÕES DE STATUS (PISCANTES) */
    @keyframes blink-green { 50% { background-color: #28a745; box-shadow: 0 0 25px #28a745; } }
    @keyframes blink-red { 50% { background-color: #dc3545; box-shadow: 0 0 25px #dc3545; } }
    @keyframes blink-yellow { 50% { background-color: #ffc107; box-shadow: 0 0 25px #ffc107; } }

    /* CLASSES DINÂMICAS PARA OS BOTÕES INDICADORES */
    .btn-ligado button { animation: blink-green 1s infinite !important; background-color: #1e7e34 !important; }
    .btn-desligado button { animation: blink-red 1s infinite !important; background-color: #bd2130 !important; }
    .btn-repouso button { animation: blink-yellow 1s infinite !important; background-color: #d39e00 !important; color: black !important; }

    .status-ok-box { 
        color: #28a745; font-weight: bold; padding: 20px; border: 2px solid #28a745; 
        border-radius: 8px; text-align: center; background-color: #e8f5e9; font-size: 22px; width: 100%;
    }
    .status-alert-box { 
        color: #dc3545; font-weight: bold; padding: 20px; border: 2px solid #dc3545; 
        border-radius: 8px; text-align: center; background-color: #fdecea; font-size: 22px; width: 100%;
    }

    .home-card { 
        background-color: #ffffff; padding: 25px; border-radius: 15px; 
        box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-top: 5px solid #00458d; text-align: center; height: 100%; 
    }
    .gauge-card { 
...
if not st.session_state["logado"]:
    ...
    menu = st.sidebar.radio("Navegação:", ["🏠 Home", "🕹️ Acionamento", "🌡️ Medição", "📊 Relatórios", "🛠️ Diagnóstico", "👥 Gestão de Usuários"])
    ...
    if menu == "🏠 Home":
        ...
        if st.session_state["modo_operacao"] == "MANUAL":
            c1, c2, c3 = st.columns(3)
            with c1:
                if status_real == "ON": st.markdown('<div class="btn-ligado">', unsafe_allow_html=True)
                if st.button("LIGAR"): db.reference("controle/led").set("ON"); registrar_evento("LIGOU"); st.rerun()
                if status_real == "ON": st.markdown('</div>', unsafe_allow_html=True)
            with c2:
                if status_real == "REPOUSO": st.markdown('<div class="btn-repouso">', unsafe_allow_html=True)
                if st.button("REPOUSO"): db.reference("controle/led").set("REPOUSO"); registrar_evento("REPOUSO"); st.rerun()
                if status_real == "REPOUSO": st.markdown('</div>', unsafe_allow_html=True)
            with c3:
                if status_real == "OFF": st.markdown('<div class="btn-desligado">', unsafe_allow_html=True)
                if st.button("DESLIGAR"): db.reference("controle/led").set("OFF"); registrar_evento("DESLIGOU"); st.rerun()
                if status_real == "OFF": st.markdown('</div>', unsafe_allow_html=True)
        else:
            ...
            if not st.session_state["ciclo_ativo"]:
                ...
                if st.button("⏹️ PARAR CICLO"): st.session_state["ciclo_ativo"] = False; db.reference("controle/led").set("OFF"); st.rerun()
                restante = t_auto - ((time.time() - st.session_state["hora_inicio_ciclo"]) / 60)
                st.success(f"⚡ Operando: {restante:.2f} min restantes"); time.sleep(1); st.rerun()

    elif menu == "🌡️ Medição":
        st.header("Telemetria Industrial")
        t, u = db.reference("sensor/temperatura").get() or 0, db.reference("sensor/umidade").get() or 0
        col1, col2 = st.columns(2)
        with col1: st.markdown(f'<div class="gauge-card">Temperatura (°C)<div class="gauge-value">{t}</div></div>', unsafe_allow_html=True)
        with col2: st.markdown(f'<div class="gauge-card">Umidade (%)<div class="gauge-value">{u}</div></div>', unsafe_allow_html=True)
        ...
        if logs:
            ...
            st.download_button("📥 BAIXAR CSV", df.to_csv(index=False).encode('utf-8'), "historico.csv", "text/csv")
        ...
        with st.form("cad_u"):
            ...
            if st.form_submit_button("CADASTRAR"):
                ...
                st.rerun()
        usrs = db.reference("usuarios_autorizados").get()
        if usrs: st.table(pd.DataFrame(list(usrs.values()))[["nome", "login", "senha"]])

# ASB AUTOMAÇÃO INDUSTRIAL - v74.0
